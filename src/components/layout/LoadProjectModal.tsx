...
export function LoadProjectModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Modal title="Load Project" onClose={onClose} width={480}>
      {projects.length === 0 ? (
        <div className="py-8 text-center text-sm text-slate-500">
          <Icon name="folder" size={28} className="mx-auto mb-2 opacity-50" />
          No saved projects yet.
          <div className="mt-1 text-xs text-slate-600">Use “Save” in the top bar to store one here.</div>
        </div>
      ) : (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[11px] text-slate-500">
                  ...
                  {p.id === currentId && <span className="ml-2 text-accent-cyan">• current</span>}
                </div>
              </div>
              <button
