...
export function AudioControls() {
  ...
  return (
    <div className="flex items-center gap-2">
      <button className="btn" onClick={() => inputRef.current?.click()} title={t('audio.import.title')}>
        <Icon name="music" size={14} /> {t('audio.import')}
      </button>
      <input
        ref={inputRef}
        type="file"
      ...
      {hasAudio && audioName && (
        <div className="flex items-center gap-1.5 rounded-md border border-ink-700 bg-ink-850 px-2 py-1 text-[11px] text-slate-300">
          ...
          <button className="text-slate-500 hover:text-rose-300" onClick={clearAudio} title={t('audio.remove')}>
