...
function ObjectInspector({ object }: { object: SceneObject }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="-mt-2 text-[11px] uppercase tracking-wider text-slate-500">{entry.label}</div>

      <Vec3Field label="Position" value={object.position} onChange={(position) => set({ position })} />
      <Vec3Field
        label="Rotation (rad)"
      ...
      <div className="grid grid-cols-2 gap-2">
        <NumberField label="Scale" value={object.scale} step={0.05} min={0.05} onChange={(scale) => set({ scale })} />
        <SliderField label="Intensity" value={object.intensity} min={0} max={2} onChange={(intensity) => set({ intensity })} />
      </div>

      <ColorField label="Color" value={object.color} onChange={(color) => set({ color })} />
      ...
      {entry.emitsBeam && (
        <>
          ...
          />
          <Vec3Field label="Target Point" value={object.target} onChange={(target) => set({ target })} />
        </>
      )}
...
function EmptyState() {
  return (
    <div className="flex h-full flex-col items-center justify-center px-6 text-center">
      ...
      <p className="text-sm font-medium text-slate-400">Select an object to edit its properties</p>
      <p className="mt-1 text-xs text-slate-600">
        Click an object in the 3D scene, or a clip on the timeline.
      </p>
    </div>
  );
}
